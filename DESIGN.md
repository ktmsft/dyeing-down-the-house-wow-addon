...
Written for someone with no World of Warcraft background.
...
Six files, loaded in dependency order via a manifest
([DyingDownTheHouse.toc](DyingDownTheHouse.toc)):
...
next. So the scanner is a queue-driven state machine
([Core.lua:651-801](Core.lua#L651-L801)) advanced by three different event types.
...
[tests/test_dyingdownthehouse.lua](tests/test_dyingdownthehouse.lua) is a stub
...
synthetic dataset injected, **188 assertions** under LuaJIT. Covers herb-overlap
...
emits `dist/DyingDownTheHouse-0.1.0.zip` in the layout CurseForge expects.
